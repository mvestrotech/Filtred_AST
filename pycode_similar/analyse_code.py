...
class BaseNodeNormalizer(ast.NodeTransformer):
    ...
    def visit_Constant(self, node):
        ...
        # for Python 3.8
        dummy_value = '__analyse_code_dummy_value__'
        if type(node) == str:
...
class FuncDiffInfo(object):
    """
    An object stores the result of code python code compared to referenced python code.
    """
    ...
    @property
    def similar_percent(self):
        return 0 if self.total_count == 0 else (self.similar_count / float(self.total_count))

    def __str__(self):
        if isinstance(self.info_ref, FuncInfo) and isinstance(self.info_code, FuncInfo):
            return ''.format(self.similar_percent,
                                                          self.info_ref.func_name + '<' + str(
                                                                  self.info_ref.func_node.lineno) + ':' + str(
                                                                  self.info_ref.func_node.col_offset) + '>',
                                                          self.info_code.func_name + '<' + str(
                                                                  self.info_code.func_node.lineno) + ':' + str(
                                                                  self.info_code.func_node.col_offset) + '>')
        return ''.format(0, None, None)
...
def detect(pycode_string_list, diff_method=UnifiedDiff, keep_prints=False, module_level=False):
    ...
    for index_code, func_info_code in func_info_list[1:]:
        func_ast_diff_list = []

        for fi1 in func_info_ref:
            min_diff_value = int((1 << 31) - 1)
            min_diff_func_info = None
            for fi2 in func_info_code:
                dv = diff_method.diff(fi1, fi2)
                if dv < min_diff_value:
                    min_diff_value = dv
                    min_diff_func_info = fi2
                if dv == 0:  # entire function structure is same
                    break

            func_diff_info = FuncDiffInfo()
            func_diff_info.info_ref = fi1
            func_diff_info.info_code = min_diff_func_info
            func_diff_info.total_count = diff_method.total(fi1, min_diff_func_info)
            func_diff_info.similar_count = func_diff_info.total_count - min_diff_value if min_diff_func_info else 0
            func_ast_diff_list.append(func_diff_info)
        func_ast_diff_list.sort(key=operator.attrgetter('similar_percent'), reverse=True)
        ast_diff_result.append((index_code, func_ast_diff_list))
...
def summarize(func_ast_diff_list):
    ...
    sum_similar_count = sum(func_diff_info.similar_count for func_diff_info in func_ast_diff_list)
    if sum_total_count == 0:
        sum_similar_percent = 0
    else:
        sum_similar_percent = sum_similar_count / float(sum_total_count)
    return sum_similar_percent, sum_similar_count, sum_total_count
...
def main():
    ...
    parser = ArgParser(description='A simple similar detection tool for python code')
    ...
    parser.add_argument('-p', type=check_percentage_limit, default=0.5,
                        help='if similar percentage of the function >= value then output detail (default: 0.5)')
    ...
    for index, func_ast_diff_list in results:
        print('program1: {}'.format(pycode_list[0][0]))
        print('program2: {}'.format(pycode_list[index][0]))
        sum_similar_percent, sum_similar_count, sum_total_count = summarize(func_ast_diff_list)
        print('{:.2f} %  of similarity between the two programs.'.format(
            sum_similar_percent * 100,
            sum_similar_count,
            sum_total_count,
        ))
        ...
        for func_diff_info in func_ast_diff_list:
            if len(func_diff_info.info_ref.func_ast_lines) >= args.l and func_diff_info.similar_percent >= args.p:
